...
// 全局日志控制
let _isHttpLogEnable = true;
let _isWsLogEnable = true;
export const enableHttpLog = (enabled: boolean) => { _isHttpLogEnable = enabled; };
export const enableWsLog = (enabled: boolean) => { _isWsLogEnable = enabled; };
...
export class HttpApiClient {
  ...
  constructor(options: HttpApiClientOptions = {}) {
    ...
    const defaultConfig: AxiosRequestConfig = {
      timeout: 10000,
      withCredentials: false, // 跨域请求不携带credentials
      headers: {
  ...
  private setupInterceptors() {
    ...
    this.instance.interceptors.request.use(
      (config: InternalAxiosRequestConfig) => {
        ...
        if (_isHttpLogEnable) { this.printRequest(config); }
        return config;
      },
      (error: AxiosError) => {
        if (!enableHttpLog) { console.error(`[http] Request setup failed:`, error); }
    ...
    this.instance.interceptors.response.use(
      (response: AxiosResponse) => {
        if (_isHttpLogEnable) { this.printResponse(response); }
        return response;
      },
      (error: AxiosError) => {
...
export class WsApiClient {
  ...
  async subscribe(topics: string[]): Promise<(ReadableStream<any> | null)[]> {
    if (_isWsLogEnable) console.log('[websocket] 订阅topics: ', topics)
    ...
    if (!this.isConnected) {
      await this._connect();
      if (_isWsLogEnable) console.log(`[websocket] websocket 连接成功 ==> ${this.url}`);
    }
    ...
    try {
      ...
      if (_isWsLogEnable) console.log(`[websocket] subscribe ${topics} error`, e);
    }
  ...
  async unsubscribe(topics: string[]): Promise<void> {
    if (_isWsLogEnable) console.log('[websocket] 取消订阅topics: ', topics)
    if (!this.url) throw new Error('WebSocket URL not set');
  ...
  async sendMessage(msg: Record<string, any>): Promise<void> {
    ...
    if (_isWsLogEnable) console.debug('[websocket] send ==> ' + jsonString);
  ...
  private async _connect(): Promise<void> {
    ...
    this._connectionPromise = new Promise(async (resolve, reject) => {
      if (_isWsLogEnable) console.debug('[websocket] connect ==> ' + this.url);
      ...
      try {
        ...
        this._socket.onopen = () => {
          if (_isWsLogEnable) console.debug('[websocket] connected successfully');
          resolve();
        };
        ...
        if (_isWsLogEnable) console.debug('[websocket] connect failed: ' + e);
  ...
  private async _onMessage(event: MessageEvent) {
    try {
      ...
      if (_isWsLogEnable) console.debug('[websocket] recv <== ' + jsonString);
  ...
  private _onDone(): void {
    if (_isWsLogEnable) console.debug('WebSocket disconnected.');
  ...
  private _onError(error: Event): void {
    if (_isWsLogEnable) console.debug('WebSocket error: ' + error);
    this._socket = null;
    this._connectionPromise = null;
  ...
  private async _scheduleReconnect(): Promise<void> {
    ...
    if (_isWsLogEnable) console.log(`Scheduling reconnection attempt ${this._reconnectCount}/${this.retryCount} in ${delay}ms`);
    ...
    try {
      ...
      if (this.isConnected && this._topics.size > 0) {
        ...
        if (_isWsLogEnable) console.log('Resubscribing to topics:', topics);
...
export class WsApiTopic {
  ...
  private _createStream(): void {
    this._stream = new ReadableStream<any>({
      start: (controller) => {
        ...
        if (_isWsLogEnable) console.debug(`Stream created for topic: ${this.topic}`);
      },
      pull: (_controller) => {
        // Handle backpressure if needed
      },
      cancel: () => {
        if (_isWsLogEnable) console.debug(`Stream cancelled for topic: ${this.topic}`);
        this.close();
      },
    });
  ...
  complete(): void {
    ...
    this._isReady = true;
    if (_isWsLogEnable) console.debug(`Subscription confirmed for topic: ${this.topic}`);
  ...
  close(): void {
    ...
    this._isClosed = true;
    if (_isWsLogEnable) console.debug(`Closing topic: ${this.topic}`);
  ...
  reset(): void {
    ...
    if (_isWsLogEnable) console.debug(`Resetting topic: ${this.topic}`);
    this._isReady = false;
